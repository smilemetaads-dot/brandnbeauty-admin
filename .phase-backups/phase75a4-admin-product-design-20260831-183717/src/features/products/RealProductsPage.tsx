...
const ADVANCED_PRODUCT_FILTERS = PRODUCT_FILTERS.filter(
  (filter) => !PRIMARY_PRODUCT_FILTERS.includes(filter),
);
...
const getMatchesProductFilter = (product: SearchableProductRecord, filter: string) => {
  ...
  switch (filter) {
    ...
    case "Hidden":
      return product.status === "draft" || product.status === "inactive";
    case "Draft":
...
function ProductImage({
  ...
  if (!product.image) {
    return (
      <div
        className={`flex shrink-0 items-center justify-center rounded-2xl border border-dashed border-slate-200 bg-stone-50 text-[10px] font-bold text-slate-400 ${className ?? ""}`}
  ...
  return (
    <Image
      alt=""
      className={`shrink-0 rounded-2xl border border-slate-200 bg-stone-100 object-cover ${className ?? ""}`}
      height={size}
      src={product.image}
      unoptimized
      width={size}
...
export function RealProductsPage({ products: initialProducts = [] }: RealProductsPageProps) {
  ...
  const [archivingProductIds, setArchivingProductIds] = useState<string[]>([]);
  ...
  const visibleCount = products.filter(
    (product) => getVisibilityLabel(product.status) === "Visible",
  ).length;
  ...
  const contentIssueCount = products.filter((product) => !product.image || !product.short_description).length;
  ...
  async function handleArchiveProduct() {
    ...
    setArchivingProductIds((current) => Array.from(new Set([...current, productId])));
    setArchiveMessage("");
    ...
    try {
      ...
      if (!response.ok || !result?.success) {
        throw new Error(result?.message ?? "Product could not be archived.");
      }

      setArchiveTarget(null);
      setArchiveMessage(result.message ?? "Product archived successfully");
      await loadProducts();
    } catch (error) {
      setArchiveMessage(
        error instanceof Error ? error.message : "Product could not be archived.",
      );
    } finally {
      setArchivingProductIds((current) => current.filter((id) => id !== productId));
    }
  }
  ...
  return (
    <AdminShell>
      <div className="space-y-6">
        ...
        <section className="flex flex-col gap-4 rounded-[2rem] border border-slate-200 bg-white px-6 py-5 shadow-sm xl:flex-row xl:items-center xl:justify-between">
          <div>
            ...
            <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-500">
              Keep every product sellable, accurate, profitable and ready for the storefront.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">

            <Link
              className="rounded-2xl bg-[#5E7F85] px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-950 focus:outline-none focus:ring-2 focus:ring-[#5E7F85]/30"
              href="/products/edit"
            >
              Add Product
            </Link>
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          />
          <AdminStatCard
            helper={`${Math.round((visibleCount / Math.max(products.length, 1)) * 100)}% of catalog`}
            icon="LV"
            index={1}
          ...
          />
          <AdminStatCard
            helper={`${lowStockCount} low · ${products.filter((product) => product.stock <= 0).length} out`}
            icon="SR"
            index={2}
            label="Stock risk"
            value={lowOrOutCount}
          />
          <AdminStatCard
            helper="Needs enrichment"
            icon="CI"
            index={3}
            label="Content issues"
            value={contentIssueCount}
          />
        </section>
        ...
        <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
          <div className="grid gap-6 p-6 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-start">
            <div>
              ...
              <p className="mt-2 max-w-3xl text-sm leading-6 text-slate-500">
                Manage product visibility, stock health, pricing, and catalog operations using the real tools connected to this store.
              </p>
            </div>

          </div>
          <div className="grid gap-3 border-t border-slate-100 bg-[#f8fbfa] p-4 md:grid-cols-3">
            <button
              className="rounded-2xl border border-slate-200 bg-white px-4 py-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#5E7F85]/40 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[#5E7F85]/30"
              onClick={() => setShowCsvImport((current) => !current)}
              type="button"
            >
              <div className="text-sm font-bold text-slate-900">Bulk Import</div>
              <div className="mt-1 text-xs leading-5 text-slate-500">Upload products and variants from CSV.</div>
            </button>
            <button
              className="rounded-2xl border border-slate-200 bg-white px-4 py-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#5E7F85]/40 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[#5E7F85]/30"
              ...
              type="button"
            >
              <div className="text-sm font-bold text-slate-900">Price & Stock CSV Update</div>
              <div className="mt-1 text-xs leading-5 text-slate-500">Preview and apply real price or stock updates.</div>
            </button>
            <button
              className="rounded-2xl border border-slate-200 bg-white px-4 py-4 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-[#5E7F85]/40 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-[#5E7F85]/30"
              ...
              type="button"
            >
              <div className="text-sm font-bold text-slate-900">Bulk Fulfillment Setup</div>
              <div className="mt-1 text-xs leading-5 text-slate-500">Review and apply supported inventory modes.</div>
            </button>
          ...
          <div className="grid gap-3 border-t border-slate-100 bg-stone-50/70 p-4 text-sm md:grid-cols-4">
            <div className="rounded-2xl bg-white px-4 py-3 text-slate-600">
              Visible products: <b className="text-slate-900">{visibleCount}</b>
            </div>
            <div className="rounded-2xl bg-white px-4 py-3 text-slate-600">
              Hidden products: <b className="text-rose-700">{hiddenCount}</b>
            </div>
            <div className="rounded-2xl bg-white px-4 py-3 text-slate-600">
              Total stock alerts: <b className="text-amber-700">{lowOrOutCount}</b>
            </div>
            <div className="rounded-2xl bg-white px-4 py-3 text-slate-600">
              Out-of-stock visible: <b className="text-rose-700">{visibleOutOfStockCount}</b>
            </div>
          </div>
        </section>
          ...
          <div className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
            <div className="border-b border-slate-100 px-5 py-4">
              ...
              <div className="mt-4 flex flex-col gap-2 xl:flex-row xl:items-center">
                <div className="relative xl:w-[22rem] xl:shrink-0">
                  <input
                    className="w-full rounded-xl border border-slate-300 bg-stone-50 px-3 py-2.5 pl-9 pr-16 text-sm outline-none transition placeholder:text-slate-400 focus:border-[#5E7F85] focus:ring-2 focus:ring-[#5E7F85]/20"
                  ...
                  {searchQuery ? (
                    <button
                      className="absolute right-2 top-1/2 -translate-y-1/2 rounded-full px-2.5 py-1 text-xs font-semibold text-slate-500 transition hover:bg-white hover:text-slate-900"
                ...
                <div className="flex flex-wrap gap-2">
                  {PRIMARY_PRODUCT_FILTERS.map((item) => (
                    <button
                  ...
                  <button
                    ...
                    className="rounded-full border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-600 transition hover:bg-stone-50"
                    onClick={() => setShowMoreFilters((current) => !current)}
                    type="button"
                  >
                    {showMoreFilters ? "Fewer Filters" : "More Filters"}
                  </button>
                  {showMoreFilters || ADVANCED_PRODUCT_FILTERS.includes(activeFilter)
                    ? ADVANCED_PRODUCT_FILTERS.map((item) => (
                        <button
                          className={`rounded-full px-3 py-1.5 text-xs font-semibold ${
                            item === activeFilter
                              ? "bg-[#5E7F85] text-white"
                        ...
                        <td className="px-5 py-4">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <button
                              ...
                              disabled={archivingProductIds.includes(product.id)}
                              onClick={() => setArchiveTarget({ id: product.id, name: product.name })}
                              type="button"
                            >
                              {archivingProductIds.includes(product.id)
                                ? "Archiving..."
                                : "Archive"}
                            </button>
                          </div>
              ...
              {selectedProduct ? (
                <>
                  ...
                  <div className="mt-5 grid gap-3">
                    <Link
                      className="rounded-2xl bg-[#5E7F85] px-4 py-3 text-center text-sm font-semibold text-white transition hover:bg-slate-950 focus:outline-none focus:ring-2 focus:ring-[#5E7F85]/30"
                      href={`/products/edit?id=${selectedProduct.id}`}
                    >
                      Edit Product
                    </Link>
                    <Link
                      className="rounded-2xl border border-slate-300 bg-white px-4 py-3 text-center text-sm font-semibold text-slate-700 transition hover:bg-stone-50 focus:outline-none focus:ring-2 focus:ring-[#5E7F85]/30"
                      href="/purchases"
                    >
                      Stock Entry
                    </Link>
                    ...
                    />
                    {selectedProduct.status !== "draft" ? (<DraftArchiveButton onUpdated={loadProducts} product={selectedProduct} />) : null}
                  </div>
                </>
              ) : (
      ...
      {archiveTarget ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/35 px-4 py-6">
          <div
            ...
            aria-modal="true"
            className="w-full max-w-md rounded-[1.5rem] border border-slate-200 bg-white p-6 shadow-2xl"
            role="dialog"
          >
            <div className="text-sm font-semibold uppercase tracking-[0.16em] text-[#5E7F85]">
              Product Archive
            </div>
            <h2
            ...
            <p className="mt-3 text-sm leading-6 text-slate-600">
              This product will be removed from the normal catalog view, while existing order history will remain unchanged.
            </p>
            <div className="mt-3 rounded-2xl bg-stone-50 px-4 py-3 text-sm font-semibold text-slate-700">
              {archiveTarget.name}
            </div>
            <div className="mt-6 flex flex-wrap justify-end gap-2">
              <button
                className="rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-stone-50 disabled:cursor-not-allowed disabled:opacity-60"
                disabled={archivingProductIds.includes(archiveTarget.id)}
              ...
              <button
                className="rounded-2xl bg-[#5E7F85] px-4 py-3 text-sm font-semibold text-white transition hover:bg-slate-950 disabled:cursor-not-allowed disabled:bg-slate-300"
                disabled={archivingProductIds.includes(archiveTarget.id)}
                onClick={() => void handleArchiveProduct()}
                type="button"
              >
                {archivingProductIds.includes(archiveTarget.id)
                  ? "Archiving..."
                  : "Archive Product"}
              </button>
            </div>
