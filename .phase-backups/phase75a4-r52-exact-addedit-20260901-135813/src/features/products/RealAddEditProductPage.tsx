...
export function RealAddEditProductPage(_props: RealAddEditProductPageProps) {
  ...
  const publishChecks = [
    ...
    {
      label: "At least one valid variant",
      ok: !isVariantProduct || variants.length > 0,
    },
    { label: "Selling Price", ok: Number(previewSalePrice) > 0 },
    {
      label: "Stock or Available-on-Order",
      ok: inventoryMode === "on_demand" || Number(previewStockQty) >= 0,
    },
  ...
  const saveProductToBackend = async (nextStatus = "Draft") => {
    ...
    try {
      ...
      setIsDirty(false);
      setSaveStatus(
        nextStatus === "Visible" ? "Visible just now" : "Saved just now",
      );
          ...
          <aside className="h-fit space-y-4 xl:sticky xl:top-[94px]">
            <section className="rounded-2xl border border-[#e2e8e5] bg-white p-4">
              <div className="flex items-start justify-between gap-4">
                ...
                >
                  {publishChecks.length - completedPublishChecks}
                </span>
              </div>
              <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#edf1ef]">
                <div
              ...
              <div className="mt-4 max-h-52 space-y-1.5 overflow-y-auto pr-1">
                {publishChecks.map((check) => (
                  <button
                    ...
                    onClick={() =>
                      setActiveEditorSection(
                        check.label === "Main image"
                          ? "images"
                          : check.label.includes("Price") ||
                              check.label.includes("Stock")
                            ? "pricing"
                            : check.label.includes("variant")
                              ? "variants"
                              : "basic",
                      )
                    }
                  ...
                  >
                    <span
                      className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full ${check.ok ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}
                    >
                      {check.ok ? "✓" : "!"}
                    </span>
