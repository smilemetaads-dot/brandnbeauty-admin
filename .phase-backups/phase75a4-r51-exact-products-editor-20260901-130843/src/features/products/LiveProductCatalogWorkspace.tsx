...
function LegacyLiveProductEditorWorkspace({ onNavigate }: { onNavigate: (page: string) => void }) {
  const [products, setProducts] = useState<CatalogProduct[]>([]);
  ...
  const readinessChecks = [
    ...
    ["Customer description", Boolean(draft.shortDescription || draft.description), "content"],
  ] as const;
  ...
  return <div className="space-y-5">
    <header className="flex flex-col gap-5 border-b border-[#dfe6e3] pb-5 xl:flex-row xl:items-end xl:justify-between">
      <div>
        <button className="text-[9px] font-bold text-[#547874]" onClick={() => onNavigate("Products")} type="button">← Back to products</button>
        <div className="mt-4 flex items-center gap-2"><p className="text-[8px] font-extrabold uppercase tracking-[.18em] text-[#547874]">Catalog editor</p><span className={`rounded-full px-2 py-1 text-[7px] font-bold ${draft.status === "active" ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{draft.status === "active" ? "Live product" : "Draft product"}</span></div>
        <h1 className="mt-2 text-[28px] font-bold tracking-[-.045em] text-[#17231f]">{draft.name || "Add new product"}</h1>
        <p className="mt-1 text-[8px] font-medium text-[#82908a]">{draft.sku || "SKU will be added"} · {draft.draftId ? `Recoverable draft #${draft.draftId}` : draft.id ? "Live product loaded" : "New catalog record"}</p>
      </div>
      <div className="flex flex-wrap gap-2">
        {draft.draftId ? <button className="rounded-xl border border-rose-100 bg-white px-4 py-3 text-[9px] font-bold text-rose-700 disabled:opacity-40" disabled={isSaving} onClick={() => void discard()} type="button">Discard draft</button> : null}
        <button className="rounded-xl border border-[#c8d5d1] bg-white px-4 py-3 text-[9px] font-bold text-[#426d72] disabled:opacity-40" disabled={!ready || isSaving || isLoading} onClick={() => void saveDraft()} type="button">{isSaving ? "Saving..." : "Save draft"}</button>
        <button className="rounded-xl bg-[#426d72] px-5 py-3 text-[9px] font-bold text-white disabled:opacity-40" disabled={!ready || isSaving || isLoading} onClick={() => setPublishOpen(true)} type="button">Review & publish</button>
      </div>
    </header>
    {loadError ? <div className="rounded-xl border border-rose-200 bg-rose-50 px-4 py-3 text-[9px] font-bold text-rose-700">{loadError}</div> : null}
    {isLoading ? <div className="rounded-2xl border bg-white p-16 text-center text-[9px] font-bold text-[#839089]">Loading product editor...</div> : <section className="grid gap-4 xl:grid-cols-[220px_minmax(0,1fr)_300px]">
      <aside className="self-start rounded-2xl border border-[#dfe6e3] bg-white p-3 xl:sticky xl:top-5">
        <div className="px-3 py-2"><p className="text-[8px] font-extrabold uppercase tracking-[.14em] text-[#8b9691]">Product sections</p><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-[#edf1ef]"><div className="h-full rounded-full bg-[#5e8588]" style={{ width: `${readinessPercent}%` }}/></div><div className="mt-2 flex justify-between text-[7px] font-bold text-[#8b9691]"><span>Completeness</span><span>{readinessPercent}%</span></div></div>
        <nav className="mt-2 space-y-1">{sections.map(([id, label, helper]) => <button className={`w-full rounded-xl px-3 py-3 text-left ${activeSection === id ? "bg-[#eaf2f2] text-[#365f67]" : "text-[#66756e] hover:bg-[#f7f9f8]"}`} key={id} onClick={() => setActiveSection(id)} type="button"><b className="block text-[9px]">{label}</b><span className="mt-1 block text-[7px] opacity-70">{helper}</span></button>)}</nav>
      </aside>
      <main className="min-w-0 space-y-4">
        {activeSection === "general" ? <article className="rounded-2xl border border-[#dfe6e3] bg-white p-6"><p className="text-[8px] font-extrabold uppercase tracking-[.14em] text-[#547874]">Product identity</p><h2 className="mt-2 text-[17px] font-bold text-[#26362f]">General information</h2><p className="mt-1 text-[8px] text-[#82908a]">Core storefront identity and customer-facing summary.</p><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="Product name *"><input className={inputClass} onChange={(event) => { const name = event.target.value; change({ name, slug: draft.slug || slugify(name) }); }} value={draft.name}/></Field><Field label="Product type"><select className={inputClass} defaultValue="single"><option value="single">Simple product</option><option value="variant">Variant product</option></select></Field><Field label="Storefront slug"><input className={inputClass} onChange={(event) => change({ slug: slugify(event.target.value) })} value={draft.slug}/></Field><Field label="Publication status"><select className={inputClass} onChange={(event) => change({ status: event.target.value as ProductStatus })} value={draft.status}><option value="draft">Draft / hidden</option><option value="active">Active / visible</option><option value="inactive">Inactive</option><option value="out_of_stock">Out of stock</option></select></Field><div className="sm:col-span-2"><Field label="Short description"><textarea className={textareaClass} onChange={(event) => change({ shortDescription: event.target.value })} value={draft.shortDescription}/></Field></div></div></article> : null}
        {activeSection === "media" ? <article className="rounded-2xl border border-[#dfe6e3] bg-white p-6"><p className="text-[8px] font-extrabold uppercase tracking-[.14em] text-[#547874]">Product media</p><h2 className="mt-2 text-[17px] font-bold text-[#26362f]">Primary image</h2><div className="mt-5 grid gap-5 lg:grid-cols-[260px_1fr]"><div className="flex h-64 items-center justify-center overflow-hidden rounded-2xl bg-[#eef4f2] text-[#426d72]" style={productImageUrl(draft.image) ? { backgroundImage: `url(${productImageUrl(draft.image)})`, backgroundPosition: "center", backgroundRepeat: "no-repeat", backgroundSize: "contain" } : undefined}>{!productImageUrl(draft.image) ? <Icon name="image" size={34}/> : null}</div><div><Field label="Image URL"><input className={inputClass} onChange={(event) => change({ image: event.target.value })} value={draft.image}/></Field><label className="mt-4 flex h-11 cursor-pointer items-center justify-center gap-2 rounded-xl border border-[#cddbd7] bg-[#f7faf9] text-[9px] font-bold text-[#416764]"><Icon name="upload" size={13}/>{isUploading ? "Uploading..." : "Upload JPG, PNG or WebP"}<input accept="image/jpeg,image/png,image/webp" className="hidden" disabled={isUploading} onChange={(event) => { const file = event.target.files?.[0]; if (file) void uploadImage(file); event.currentTarget.value = ""; }} type="file"/></label><p className="mt-3 text-[8px] leading-4 text-[#82908a]">The existing media upload and product image storage remain connected.</p></div></div></article> : null}
        {activeSection === "pricing" ? <article className="rounded-2xl border border-[#dfe6e3] bg-white p-6"><p className="text-[8px] font-extrabold uppercase tracking-[.14em] text-[#547874]">Commercial information</p><h2 className="mt-2 text-[17px] font-bold text-[#26362f]">Pricing</h2><div className="mt-5 grid gap-4 sm:grid-cols-3"><Field label="Selling price *"><input className={inputClass} min="0" onChange={(event) => change({ price: Math.max(0, Number(event.target.value) || 0) })} type="number" value={draft.price}/></Field><Field label="Regular / compare price"><input className={inputClass} min="0" onChange={(event) => change({ comparePrice: event.target.value === "" ? null : Math.max(0, Number(event.target.value) || 0) })} type="number" value={draft.comparePrice ?? ""}/></Field><Field label="Cost price"><input className={inputClass} min="0" onChange={(event) => change({ costPrice: event.target.value === "" ? null : Math.max(0, Number(event.target.value) || 0) })} type="number" value={draft.costPrice ?? ""}/></Field></div></article> : null}
        {activeSection === "inventory" ? <article className="rounded-2xl border border-[#dfe6e3] bg-white p-6"><p className="text-[8px] font-extrabold uppercase tracking-[.14em] text-[#547874]">Stock policy</p><h2 className="mt-2 text-[17px] font-bold text-[#26362f]">Inventory & availability</h2><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="SKU"><input className={inputClass} onChange={(event) => change({ sku: event.target.value })} value={draft.sku}/></Field><Field label="Current stock *"><input className={inputClass} min="0" onChange={(event) => change({ stock: Math.max(0, Math.floor(Number(event.target.value) || 0)) })} type="number" value={draft.stock}/></Field><Field label="Low-stock alert"><input className={inputClass} min="0" onChange={(event) => change({ lowStockThreshold: Math.max(0, Math.floor(Number(event.target.value) || 0)) })} type="number" value={draft.lowStockThreshold}/></Field><label className="mt-5 flex h-10 items-center justify-between rounded-xl border border-[#dce4e0] px-3 text-[8.5px] font-bold text-[#596861]"><span>Featured product</span><input checked={draft.featured} className="h-4 w-4 accent-[#426d72]" onChange={(event) => change({ featured: event.target.checked })} type="checkbox"/></label></div><div className="mt-5 rounded-xl bg-amber-50 p-4 text-[8px] leading-4 text-amber-800">Stock receipts and adjustments should continue through Inventory so every movement keeps its ledger history.</div></article> : null}
        {activeSection === "organization" ? <article className="rounded-2xl border border-[#dfe6e3] bg-white p-6"><p className="text-[8px] font-extrabold uppercase tracking-[.14em] text-[#547874]">Catalog placement</p><h2 className="mt-2 text-[17px] font-bold text-[#26362f]">Organization</h2><div className="mt-5 grid gap-4 sm:grid-cols-2"><Field label="Brand"><input className={inputClass} onChange={(event) => change({ brand: event.target.value })} value={draft.brand}/></Field><Field label="Category"><input className={inputClass} onChange={(event) => change({ category: event.target.value })} value={draft.category}/></Field></div></article> : null}
        {activeSection === "content" ? <article className="rounded-2xl border border-[#dfe6e3] bg-white p-6"><p className="text-[8px] font-extrabold uppercase tracking-[.14em] text-[#547874]">Customer-facing copy</p><h2 className="mt-2 text-[17px] font-bold text-[#26362f]">Product content</h2><div className="mt-5 space-y-4"><Field label="Short description"><textarea className={textareaClass} onChange={(event) => change({ shortDescription: event.target.value })} value={draft.shortDescription}/></Field><Field label="Full description"><textarea className={`${textareaClass} min-h-56`} onChange={(event) => change({ description: event.target.value })} value={draft.description}/></Field></div></article> : null}
        {activeSection === "seo" ? <article className="rounded-2xl border border-[#dfe6e3] bg-white p-6"><p className="text-[8px] font-extrabold uppercase tracking-[.14em] text-[#547874]">Search & discovery</p><h2 className="mt-2 text-[17px] font-bold text-[#26362f]">SEO settings</h2><div className="mt-5 space-y-4"><Field label="SEO title"><input className={inputClass} maxLength={191} onChange={(event) => change({ metaTitle: event.target.value })} value={draft.metaTitle}/></Field><Field label="Meta description"><textarea className={textareaClass} maxLength={255} onChange={(event) => change({ metaDescription: event.target.value })} value={draft.metaDescription}/></Field></div></article> : null}
      </main>
      <aside className="space-y-4 self-start xl:sticky xl:top-5">
        <article className="rounded-2xl border border-[#dfe6e3] bg-white p-5"><div className="flex items-start justify-between gap-3"><div><p className="text-[8px] font-extrabold uppercase tracking-[.14em] text-[#7d8983]">Product readiness</p><h3 className="mt-2 text-[16px] font-bold text-[#26362f]">{readinessPercent}% complete</h3></div><span className={`rounded-xl px-3 py-2 text-[9px] font-bold ${readinessPercent === 100 ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>{readinessChecks.length - completedChecks}</span></div><div className="mt-4 h-1.5 overflow-hidden rounded-full bg-[#edf1ef]"><div className="h-full rounded-full bg-[#5e8588]" style={{ width: `${readinessPercent}%` }}/></div><div className="mt-4 space-y-2">{readinessChecks.map(([label, valid, section]) => <button className="flex w-full items-center justify-between rounded-xl bg-[#f8faf9] px-3 py-2.5 text-left text-[8px] font-bold text-[#66756e]" key={label} onClick={() => setActiveSection(section)} type="button"><span>{label}</span><span className={valid ? "text-emerald-700" : "text-amber-700"}>{valid ? "✓" : "!"}</span></button>)}</div></article>
        <article className="rounded-2xl border border-[#d8e4e1] bg-[#edf5f5] p-5"><p className="text-[8px] font-extrabold uppercase tracking-[.14em] text-[#5d7775]">Storefront result</p><div className="mt-4 rounded-xl bg-white p-4"><span className="text-[7px] font-bold uppercase text-[#8b9691]">Selling price</span><b className="mt-2 block text-[21px] text-[#26362f]">{money(draft.price)}</b><p className="mt-1 text-[7.5px] text-[#82908a]">{draft.status === "active" ? "Visible when published" : "Currently not public"} · {draft.stock} in stock</p></div></article>
      </aside>
    </section>}
    {publishOpen ? <div className="fixed inset-0 z-[90] flex items-center justify-center bg-[#17231f]/40 p-4 backdrop-blur-sm" onMouseDown={(event) => { if (event.currentTarget === event.target) setPublishOpen(false); }}><div className="w-full max-w-[580px] overflow-hidden rounded-2xl bg-white shadow-[0_30px_90px_rgba(17,35,30,.24)]"><div className="flex items-start justify-between border-b border-[#e8ecea] p-5"><div><p className="text-[8px] font-bold uppercase tracking-[.14em] text-[#426d72]">Human approval required</p><h2 className="mt-1.5 text-[18px] font-bold text-[#22312b]">Publish product changes</h2><p className="mt-1 text-[8.5px] text-[#84908a]">This can change customer-visible content, price, stock and availability.</p></div><button className="flex h-9 w-9 items-center justify-center rounded-xl bg-[#f2f5f3] text-[#65736c]" onClick={() => setPublishOpen(false)} type="button"><Icon name="close" size={14}/></button></div><div className="space-y-4 p-5"><div className="grid grid-cols-3 gap-2">{[["Price", money(draft.price)], ["Stock", String(draft.stock)], ["Status", statusLabel(draft.status)]].map(([label, value]) => <div className="rounded-xl bg-[#f5f8f6] p-3" key={label}><span className="text-[7px] font-bold text-[#7d8983]">{label}</span><b className="mt-1 block truncate text-[10px] text-[#33443d]">{value}</b></div>)}</div><Field label="Publication note (optional)"><textarea className={textareaClass} onChange={(event) => setPublishReason(event.target.value)} value={publishReason}/></Field><label className="flex cursor-pointer items-start gap-3 rounded-xl border border-[#dfe6e3] p-4"><input checked={publishConfirmed} className="mt-0.5 h-4 w-4 accent-[#426d72]" onChange={(event) => setPublishConfirmed(event.target.checked)} type="checkbox"/><span className="text-[8.5px] leading-4 text-[#596962]">I reviewed product name, price, stock, status, image and customer-facing content.</span></label></div><div className="flex justify-end gap-2 border-t border-[#e8ecea] bg-[#fafbfa] p-5"><button className="h-10 rounded-xl border border-[#dce4e0] bg-white px-4 text-[9px] font-bold text-[#66756e]" onClick={() => setPublishOpen(false)} type="button">Keep draft</button><button className="h-10 rounded-xl bg-[#426d72] px-4 text-[9px] font-bold text-white disabled:opacity-40" disabled={!publishConfirmed || isSaving} onClick={() => void publish()} type="button">{isSaving ? "Publishing..." : "Publish product"}</button></div></div></div> : null}
    {notice ? <div className="fixed bottom-6 right-6 z-[100] max-w-sm rounded-xl bg-[#335e63] px-4 py-3 text-[9.5px] font-bold leading-5 text-white shadow-xl">{notice}</div> : null}
  </div>;
