...
export const ORDERS_FALLBACK_ENDPOINT = bnbApiUrl("manage_orders.php");
...
export function normalizeLogisticsOrder(order: ApiLogisticsOrder): LogisticsOrderRecord {
  ...
  const orderStatus = normalizeStatus(order.order_status ?? order.status, "pending");
  ...
  return {
    ...
    customer_phone: order.customer_phone ?? order.phone ?? "Not available",
    delivered_at: orderStatus === "delivered" ? (order.updated_at ?? null) : null,
    delivery_zone: order.city ?? null,
    district: order.city ?? null,
    due_amount: order.due_amount === null || order.due_amount === undefined
    ...
    shipping_address: order.shipping_address ?? order.delivery_address ?? order.address ?? null,
...
export async function fetchLogisticsOrders(signal?: AbortSignal) {
  let payload: ApiLogisticsOrder[] | null = null;
  ...
  try {
    ...
    const responsePayload = (await response.json()) as ApiLogisticsOrder[] | {
      message?: string;
      success?: boolean;
    };
  ...
  if (!payload) {
    const response = await fetch(ORDERS_FALLBACK_ENDPOINT, {
      cache: "no-store",
      headers: adminAuthHeaders(),
      signal,
    });
    const responsePayload = (await response.json().catch(() => null)) as {
      message?: string;
      orders?: ApiLogisticsOrder[];
      success?: boolean;
    } | null;

    if (!response.ok || !responsePayload?.success || !Array.isArray(responsePayload.orders)) {
      throw new Error(responsePayload?.message ?? "Packing queue request failed.");
    }

    payload = responsePayload.orders;
  }
