      ...
      <div className="grid gap-6 xl:grid-cols-[260px_minmax(0,1fr)]">
        <aside className="rounded-[1.6rem] border border-slate-200 bg-white p-3 shadow-sm xl:sticky xl:top-32 xl:self-start">
          ...
          <nav className="hidden space-y-1 xl:block" aria-label="Product editor sections">
            {editorSections.map((section, index) => <button key={section.id} type="button" onClick={() => setActiveEditorSection(section.id)} className={`w-full rounded-2xl px-4 py-3 text-left transition ${activeEditorSection === section.id ? "bg-[#5E7F85] text-white shadow-sm" : "text-slate-600 hover:bg-stone-50"}`}><span className="block text-xs font-bold uppercase tracking-[0.12em] opacity-70">{String(index + 1).padStart(2, "0")}</span><span className="mt-1 block text-sm font-bold">{section.label}</span></button>)}
          </nav>
        </aside>
