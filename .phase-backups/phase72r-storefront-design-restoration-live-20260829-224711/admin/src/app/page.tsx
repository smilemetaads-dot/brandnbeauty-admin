...
function normalizeRecentOrder(value: unknown): DashboardRecentOrder | null {
  if (!value || typeof value !== "object") return null;

  const order = value as AdminOrderRow;
...
function normalizeDashboardOrders(value: unknown): DashboardRecentOrder[] {
  const payload = value && typeof value === "object" ? (value as AdminOrdersResponse) : {};
...
function EmptyTableRow({ children }: { children: ReactNode }) {
  return (
    <tr className="border-t border-slate-100">
      <td className="px-5 py-8 text-center text-sm font-semibold text-slate-500" colSpan={5}>
        {children}
      </td>
    </tr>
  );
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function loadDashboardStats() {
      try {
        const [statsResponse, chartResponse, productsResponse, ordersResponse] = await Promise.all([
        ...
        const dashboardOrders = ordersResponse.ok
          ? normalizeDashboardOrders((await ordersResponse.json()) as unknown)
          : [];
    ...
    <AdminShell>
      <div className="space-y-5">
        <section className="overflow-hidden rounded-[2rem] border border-slate-200 bg-white shadow-sm">
          <div className="relative overflow-hidden bg-gradient-to-br from-[#5E7F85] via-[#6f949a] to-[#d9e5e1] p-5 text-white sm:p-6">
        ...
        <section className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr]">
          <AdminChartCard
            bars={chartBars.length > 0 ? chartBars : undefined}
            label="Sales Trend"
            title="Weekly Revenue Overview"
          >
            <div className="mt-4 rounded-3xl bg-stone-50 px-5 py-8 text-center">
              <div className="text-sm font-bold text-slate-800">
                Revenue trend will appear when order revenue is available.
              </div>
              <div className="mt-2 text-xs font-semibold text-slate-500">
                No sample chart data is shown on the production dashboard.
              </div>
            </div>
          </AdminChartCard>

          <AdminSectionCard
            actions={<AdminBadge tone="warn">Priority Queue</AdminBadge>}
              ...
              <AdminTable>
                ...
                <tbody>
                  {summary.recentOrders.length > 0 ? (
                      ...
                      >
                        <td className="px-5 py-4">
                          <div className="font-bold text-slate-900">
                            {order.order_number ?? `BNB-${order.id.padStart(6, "0")}`}
                          </div>
                        ...
                        <td className="px-5 py-4">
                          <Link
                            ...
                            href={`/orders/details?id=${encodeURIComponent(order.id)}`}
                          >
                            Open
                          </Link>
                        </td>
                    ...
                    <EmptyTableRow>No recent orders yet.</EmptyTableRow>
                  )}
                </tbody>
              </AdminTable>
            </div>
