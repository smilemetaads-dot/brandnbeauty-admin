...
export function RealAddEditProductPage(_props: RealAddEditProductPageProps) {
  ...
  const editorSections = [
    { id: "basic", label: "Basic Information", helper: "Name, type, brand, category and concerns." },
    { id: "pricing", label: "Pricing", helper: "Selling price, regular price and discount preview." },
    { id: "images", label: "Images", helper: "Main image and gallery controls." },
    { id: "inventory", label: "Inventory & Availability", helper: "Fulfillment, stock and availability settings." },
    { id: "content", label: "Product Content", helper: "Descriptions, benefits, ingredients, usage and FAQ." },
    ...(isVariantProduct ? [{ id: "variants", label: "Variants", helper: "Options, variant prices, stock and status." }] : []),
    { id: "seo", label: "SEO Settings", helper: "Search title, description and URL preview." },
    { id: "visibility", label: "Visibility & Publish", helper: "Draft, visible and hidden status." },
    { id: "advanced", label: "Advanced", helper: "Power-user fields that affect internal operations." },
  ];
  const [activeEditorSection, setActiveEditorSection] = useState("basic");
  const activeEditorSectionMeta = editorSections.find((section) => section.id === activeEditorSection) || editorSections[0];
  ...
  return (
    <AdminShell>
      <div className="space-y-6" onChangeCapture={markEditorDirty}>
      ...
      <section className="flex flex-col gap-5 border-b border-slate-200 pb-6 xl:flex-row xl:items-end xl:justify-between">
        <div>
          <button className="text-sm font-semibold text-[#5E7F85] transition hover:text-slate-950" onClick={() => router.push("/products")} type="button">← Back to products</button>
          <div className="mt-5 flex flex-wrap items-center gap-3">
            <div className="text-xs font-bold uppercase tracking-[0.18em] text-[#5E7F85]">Catalog Editor</div>
            <Badge tone={status === "Visible" ? "good" : status === "Archived" ? "bad" : "warn"}>{status === "Visible" ? "Live product" : status}</Badge>
          </div>
          <h1 className="mt-3 text-3xl font-bold tracking-tight text-slate-950 xl:text-4xl">{productName || "Add new product"}</h1>
          <div className="mt-2 text-xs font-semibold text-slate-500">{autoSku} · {isDirty ? "Unsaved changes" : saveStatus}</div>
        </div>
        <div className="flex flex-wrap gap-2">
          <button className="rounded-2xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-700" onClick={() => window.open(storefrontProductUrl, "_blank", "noopener,noreferrer")} type="button">Preview</button>
          <button className="rounded-2xl border border-slate-300 bg-white px-5 py-3 text-sm font-semibold text-slate-700 disabled:opacity-50" disabled={isSavingProduct || isUploadingImage} onClick={saveDraft} type="button">Save draft</button>
          <button className="rounded-2xl bg-[#365f67] px-5 py-3 text-sm font-semibold text-white disabled:bg-slate-300" disabled={isSavingProduct || isUploadingImage || status === "Archived"} onClick={() => setPublishModalOpen(true)} type="button">Review & publish</button>
        </div>
      </section>
      ...
      <div className="grid gap-6 xl:grid-cols-[240px_minmax(0,1fr)_310px]">
        <aside className="rounded-[1.6rem] border border-slate-200 bg-white p-3 shadow-sm xl:sticky xl:top-32 xl:self-start">
          ...
          <div className="hidden px-3 pb-4 pt-2 xl:block"><div className="text-xs font-bold uppercase tracking-[0.16em] text-slate-400">Product sections</div><div className="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-[#5E7F85]" style={{ width: `${readinessPercent}%` }} /></div><div className="mt-2 flex justify-between text-[10px] font-semibold text-slate-400"><span>Completeness</span><span>{readinessPercent}%</span></div></div>
          <nav className="hidden space-y-1 xl:block" aria-label="Product editor sections">
            {editorSections.map((section) => <button key={section.id} type="button" onClick={() => setActiveEditorSection(section.id)} className={`w-full rounded-2xl px-4 py-3 text-left transition ${activeEditorSection === section.id ? "bg-[#eaf2f2] text-[#365f67]" : "text-slate-600 hover:bg-stone-50"}`}><span className="block text-sm font-bold">{section.label}</span><span className="mt-1 block text-[10px] leading-4 opacity-70">{section.helper}</span></button>)}
          </nav>
        </aside>

        <div className="min-w-0 space-y-6">
          <section className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm">
            <div className="text-xs font-bold uppercase tracking-[0.16em] text-[#5E7F85]">Product identity</div>
            <h1 className="mt-2 text-2xl font-bold tracking-tight text-slate-950">{activeEditorSectionMeta.label}</h1>
            <p className="mt-2 text-sm leading-6 text-slate-500">{activeEditorSectionMeta.helper}</p>
          </section>
        <div className="space-y-6">
          <div style={{ display: activeEditorSection === "basic" ? undefined : "none" }} className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm"><div className="flex items-center justify-between gap-3"><h2 className="text-xl font-bold tracking-tight">Basic Information</h2></div>
          ...
          <div style={{ display: ["images", "content", "advanced"].includes(activeEditorSection) ? undefined : "none" }} className="rounded-[2rem] border border-slate-200 bg-white p-6 shadow-sm"><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div><h2 className="text-xl font-bold tracking-tight">{activeEditorSection === "images" ? "Images" : activeEditorSection === "advanced" ? "Advanced" : "Product Content"}</h2></div></div>
            <div style={{ display: activeEditorSection === "advanced" ? undefined : "none" }} className="mt-5 space-y-4">
            ...
            <div style={{ display: activeEditorSection === "images" ? undefined : "none" }} className="mt-5 space-y-5">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2"><div className="text-sm font-medium text-slate-600">Short Description</div><textarea value={shortDescription} onChange={(e) => setShortDescription(e.target.value)} className="h-24 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm outline-none" /></label>
                <label className="space-y-2"><div className="text-sm font-medium text-slate-600">How to Use</div><textarea value={howToUse} onChange={(e) => setHowToUse(e.target.value)} className="h-24 w-full rounded-2xl border border-slate-300 bg-white px-4 py-3 text-sm outline-none" /></label>
      ...
      <aside className="hidden space-y-5 xl:block">
        <section className="sticky top-6 rounded-[1.6rem] border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-start justify-between gap-4"><div><div className="text-xs font-bold uppercase tracking-[0.14em] text-slate-400">Product readiness</div><div className="mt-2 text-lg font-bold text-slate-900">{readinessPercent}% complete</div></div><Badge tone={publishBlocked ? "warn" : "good"}>{publishChecks.length - completedPublishChecks}</Badge></div>
          <div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-[#5E7F85]" style={{ width: `${readinessPercent}%` }} /></div>
          <div className="mt-5 space-y-2">{publishChecks.map((check) => <button className={`flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-xs font-semibold ${check.ok ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`} key={check.label} onClick={() => setActiveEditorSection(check.label === "Main image" ? "images" : check.label.includes("Price") || check.label.includes("Stock") ? "pricing" : check.label.includes("variant") ? "variants" : "basic")} type="button"><span>{check.ok ? "✓" : "!"}</span><span>{check.label}</span></button>)}</div>
          <div className="mt-5 rounded-2xl bg-[#eef5f4] p-4"><div className="text-xs font-bold uppercase tracking-[0.12em] text-[#5E7F85]">Storefront result</div><div className="mt-3 text-2xl font-bold text-slate-900">{previewSalePrice ? `Tk ${previewSalePrice}` : "Price pending"}</div><div className="mt-1 text-xs text-slate-500">{status === "Visible" ? "Visible on website" : "Visible when published"} · {previewStockQty || 0} in stock</div><button className="mt-4 w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-xs font-bold text-slate-700" onClick={() => window.open(storefrontProductUrl, "_blank", "noopener,noreferrer")} type="button">Preview customer view</button></div>
        </section>
      </aside>
    </div>
    </AdminShell>
