...
function bnbInventoryTableExists(PDO $pdo, string $tableName): bool
{
...
function bnbInventoryColumns(PDO $pdo, string $tableName): array
{
    if (!bnbInventoryTableExists($pdo, $tableName)) return [];
    return $pdo->query('SHOW COLUMNS FROM `' . str_replace('`', '``', $tableName) . '`')->fetchAll(PDO::FETCH_COLUMN) ?: [];
}

function ensureStorefrontInventoryColumns(PDO $pdo): void
{
    if (!bnbInventoryTableExists($pdo, 'products')) return;

    $productColumns = bnbInventoryColumns($pdo, 'products');
    if (!in_array('inventory_mode', $productColumns, true)) {
        $pdo->exec("ALTER TABLE products ADD COLUMN inventory_mode ENUM('stocked','on_demand') NOT NULL DEFAULT 'stocked'");
    }
    if (!in_array('availability_status', $productColumns, true)) {
        $pdo->exec("ALTER TABLE products ADD COLUMN availability_status ENUM('available','unavailable') NOT NULL DEFAULT 'available'");
    }
    if (!in_array('minimum_order_quantity', $productColumns, true)) {
        $pdo->exec('ALTER TABLE products ADD COLUMN minimum_order_quantity INT UNSIGNED NOT NULL DEFAULT 1');
    }

    if (!bnbInventoryTableExists($pdo, 'product_variants')) return;
    $variantColumns = bnbInventoryColumns($pdo, 'product_variants');
    if (!in_array('inventory_mode', $variantColumns, true)) {
        $pdo->exec("ALTER TABLE product_variants ADD COLUMN inventory_mode ENUM('stocked','on_demand') NOT NULL DEFAULT 'stocked'");
    }
    if (!in_array('availability_status', $variantColumns, true)) {
        $pdo->exec("ALTER TABLE product_variants ADD COLUMN availability_status ENUM('available','unavailable') NOT NULL DEFAULT 'available'");
    }
    if (!in_array('minimum_order_quantity', $variantColumns, true)) {
        $pdo->exec('ALTER TABLE product_variants ADD COLUMN minimum_order_quantity INT UNSIGNED NOT NULL DEFAULT 1');
    }
}
...
function ensureInventorySchema(PDO $pdo): void
{
    ...
    ensureStorefrontInventoryColumns($pdo);
...
function normalizeInventoryMode(mixed $value, string $fallback = 'stocked'): string
{
    $value = is_scalar($value) ? strtolower(trim((string) $value)) : '';
    $value = str_replace([' ', '-'], '_', $value);
    return in_array($value, ['stocked', 'on_demand'], true) ? $value : $fallback;
}

function normalizeAvailabilityStatus(mixed $value, string $fallback = 'available'): string
{
    $value = is_scalar($value) ? strtolower(trim((string) $value)) : '';
    $value = str_replace([' ', '-'], '_', $value);
    return in_array($value, ['available', 'unavailable'], true) ? $value : $fallback;
}

function normalizeMinimumOrderQuantity(mixed $value, int $fallback = 1): int
{
    $value = is_scalar($value) ? trim((string) $value) : '';
    return preg_match('/^[1-9]\d*$/', $value) ? (int) $value : max(1, $fallback);
}

function inventoryAvailabilityLabel(string $inventoryMode, string $availabilityStatus, int $stockQuantity): string
{
    if ($availabilityStatus === 'unavailable') return 'Currently Unavailable';
    if ($inventoryMode === 'on_demand') return 'Available on Order';
    return $stockQuantity > 0 ? 'In Stock' : 'Currently Unavailable';
}
