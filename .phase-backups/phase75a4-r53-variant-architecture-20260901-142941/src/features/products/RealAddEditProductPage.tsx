...
// Phase 75A.4 R5.2: exact v119 Add/Edit Product section design lock.
/* eslint-disable */
/*
...
export function RealAddEditProductPage(_props: RealAddEditProductPageProps) {
  ...
  const autoSku = sku || String(skuCounter);
  const generatedSlug = productName
    ? productName
        ...
        .replace(/^-|-$/g, "")
    : "auto-generated-slug";
  const autoSlug = storefrontSlug || generatedSlug;
  ...
  const publishChecks = [
    ...
    { label: "Unique storefront slug", ok: Boolean(autoSlug), required: true, section: "basic" },
    ...
    { label: "Purchase cost / COGS", ok: Number(costPrice) > 0, required: true, section: "pricing" },
    ...
    { label: "Unique SKU", ok: Boolean(autoSku.trim()), required: true, section: "inventory" },
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            <label className={exactLabelClass}>
              ...
              <div className="relative">
                ...
                <input
                  className={`${exactInputClass} pl-7`}
                  min="0"
          ...
          <div className="grid gap-3 sm:grid-cols-3">
            {[
              ...
              >
                ...
                <div className="mt-2 text-[14px] font-bold text-[#33453d]">
                  {value}
                </div>
              </div>
            ))}
        ...
        >
          <ExactToggle
            ...
            onChange={(value) => {
              ...
              if (value && !variants.length) addVariant();
            }}
          />
          {!isVariantProduct ? (
            ...
            <div className="overflow-hidden rounded-xl border border-[#e1e7e4]">
              <div className="grid grid-cols-[1.2fr_1fr_.8fr_.55fr_.45fr_.3fr] gap-2 bg-[#f6f8f7] px-3 py-2 text-[6.5px] font-bold uppercase tracking-[0.08em] text-[#85918b]">
                <span>Option</span>
                <span>SKU</span>
                <span>Price</span>
                <span>Stock</span>
              ...
              {variants.map((variant, index) => (
                <div
                  className="grid grid-cols-[1.2fr_1fr_.8fr_.55fr_.45fr_.3fr] items-center gap-2 border-t border-[#edf0ee] px-3 py-3"
                  key={variant.id || index}
                >
                  <input
                    className="h-9 min-w-0 rounded-lg border border-[#dce4e0] px-2 text-[7.5px] outline-none"
                    onChange={(event) =>
                      updateVariantField(index, "option", event.target.value)
                    }
                    placeholder="300 ml"
                  ...
                  <input
                    ...
                    min="0"
                    onChange={(event) => {
                      updateVariantField(index, "sale", event.target.value);
                      if (!variant.regular) {
                        updateVariantField(
                          index,
                          "regular",
                    ...
                    type="number"
                    value={variant.sale || variant.regular}
                  />
              ...
              <div className="flex items-center justify-between border-t border-[#edf0ee] bg-[#fbfcfb] px-3 py-3">
                <span className="text-[7px] font-semibold text-[#7b8982]">
                  ...
                  {Number(previewStockQty || 0).toLocaleString()} total units
                </span>
                <button
                  className="h-9 rounded-xl border border-[#cfdcd8] bg-white px-3 text-[8px] font-bold text-[#3b646d]"
                  onClick={addVariant}
                  type="button"
                >
                  Add variant
