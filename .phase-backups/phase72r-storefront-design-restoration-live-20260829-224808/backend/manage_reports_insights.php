...
try {
    if (($_SERVER['REQUEST_METHOD'] ?? 'GET') !== 'GET') {
        ...
        echo json_encode(['success' => false, 'message' => 'Reports & Insights is read-only.']);
        exit;
    }
    $pdo = getDatabaseConnection();
    echo json_encode(array_merge(['success' => true, 'module' => 'reports-insights'], reportsInsightsState($pdo, $_GET['period'] ?? '30D')), JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE);
} catch (Throwable $error) {
    error_log('manage_reports_insights.php: ' . $error->getMessage());
    http_response_code(500);
    echo json_encode(['success' => false, 'module' => 'reports-insights', 'message' => 'Reports & Insights is temporarily unavailable.']);
}
